...
class RushHour:
    ...
    def solution(niveau, return_graph=False, verbeux=False):
        ...
        tmp = 0
        if verbeux==True:
            tq = tqdm.tqdm() # tqdm.tqdm_notebook()
        while possible:
            poss=possible.popleft()
            s= poss[0]
            tot =poss[1]
            for key, elem in s.voisins().items():
                if elem in pred:
                    continue
                if verbeux:
                    tq.update()

                possible.append((elem, tot+1))
                pred[elem] = key, s
                if elem.est_gagnant():
                    if return_graph:
                        return pred
